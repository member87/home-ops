...
13. **Home Assistant** (namespace: `home-assistant`)
    - Home automation platform with integrated Matter/Thread stack
    - Uses `hostNetwork: true` for IPv6 Thread routing and mDNS discovery
    - **Containers** (single pod):
      - **Home Assistant**: `ghcr.io/home-assistant/home-assistant:2026.4.1` - Home automation (port 8123)
      - **OTBR**: `homeassistant/amd64-addon-otbr:2.16.6` - OpenThread Border Router (port 8081)
      - **Matter Server**: `ghcr.io/matter-js/python-matter-server:8.1.2` - Matter controller (port 5580)
    - **Hardware**: SLZB dongle at `192.168.1.226:6638` (TCP serial bridge)
    - **URL**: `https://home.lab.jackhumes.com`
    - **Architecture**:
      ```
      Home Assistant --> Matter Server (localhost:5580) --> OTBR (localhost:8081) --> SLZB Dongle --> Thread Mesh
      ```
    - **Key Files**:
      - `apps/home-assistant/deployment.yaml` - Consolidated deployment with all 3 containers
      - `apps/home-assistant/otbr-scripts.yaml` - OTBR entrypoint and TCP bridge scripts
      - `apps/home-assistant/sealedsecret-thread-dataset.yaml` - Encrypted Thread network credentials
      - `apps/home-assistant/configmap.yaml` - HA config bootstrap
    - **Thread Dataset** (stored in SealedSecret `thread-dataset`):
      ...
      - Critical for disaster recovery - without it, all Thread devices must be factory-reset
    - **OTBR Auto-Initialization** (via entrypoint in `otbr-scripts.yaml`):
    ...
    - **Home Assistant Integrations** (configured via UI, stored in PVC):
      - Matter: `http://localhost:5580/ws`
      - OTBR: `http://localhost:8081`
      - Thread: Auto-discovered after OTBR setup
    - **Troubleshooting**:
      - Check OTBR state: `kubectl exec -n home-assistant deployment/home-assistant -c otbr -- /opt/otbr-beta/sbin/ot-ctl state`
      - Check commissioner: `kubectl exec -n home-assistant deployment/home-assistant -c otbr -- /opt/otbr-beta/sbin/ot-ctl commissioner state`
      - Check OTBR logs: `kubectl logs -n home-assistant deployment/home-assistant -c otbr --tail=50`
      - Check Matter Server logs: `kubectl logs -n home-assistant deployment/home-assistant -c matter-server --tail=50`
