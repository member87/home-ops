...
resource "terraform_data" "edge_config" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "timeout 60 sh -c 'until curl -fsS http://127.0.0.1:9100/metrics >/dev/null; do sleep 2; done'",
      "timeout 60 sh -c 'until curl -fsS http://127.0.0.1:9090/-/ready >/dev/null; do sleep 2; done'",
